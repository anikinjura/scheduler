...
4.  **Проверка расписания (`scheduler_runner/utils/timing.py`)**:
    -   **`daily`**: задача выполняется в указанный час (например, `time: "14:00"` → выполнение в любом времени между 14:00 и 14:59).
    -   **`hourly`**: задача выполняется каждый час при вызове планировщика. Опционально может быть ограничен интервалом через `time_window: "09:00-21:00"` (задача выполняется только в рабочее время).
...
**Пример hourly-задачи с ограничением по времени (`time_window`):**
```python
# scheduler_runner/tasks/cameras/config/scripts/cloudmonitor_config.py
...
SCHEDULE = [
    {
        ...
        "args": [],
        "schedule": "hourly",
        "time_window": "09:00-21:00",  # Выполняется только с 9:00 до 20:59
        "user": "operator"
    }
]
```
...
-   `time_window` (str, optional): Временное окно для `hourly` расписания (формат `HH:MM-HH:MM`). Ограничивает выполнение задач рабочим интервалом. Например, `"09:00-21:00"` означает, что задача будет выполняться только в период с 9:00 до 20:59.
