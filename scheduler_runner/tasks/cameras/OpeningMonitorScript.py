...
Скрипт для определения времени начала работы объекта по видеофайлам и времени включения компьютера.

- Сканирует одну или несколько директорий с видеозаписями (`SEARCH_DIRS`, fallback: `SEARCH_DIR`).
- Ищет файлы, созданные сегодня в заданном временном интервале (например, с 08:00 до 10:00).
- Определяет время по имени файла, поддерживая форматы:
  - `ЧЧ-ММ-СС.jpg` (UNV),
  - `..._unix-timestamp.mp4` (Xiaomi).
- Получает время включения компьютера из системных журналов.
- Выполняет комбинированный анализ и отправляет уведомление в Telegram.
