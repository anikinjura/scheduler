...
### Новое поведение при провале навигации
- Если `navigate_to_target()` вернул `False` и причина сбоя `_last_navigation_failure_reason == "login_redirect"`, метод поднимает исключение:
  - `AUTH_REQUIRED: redirected_to_login`
- Это исключение обрабатывается уровнем выше как критический признак потери сессии.

### Практический эффект
- Ошибка авторизации отделяется от обычного навигационного сбоя и не маскируется как «обычный фейл шага».
