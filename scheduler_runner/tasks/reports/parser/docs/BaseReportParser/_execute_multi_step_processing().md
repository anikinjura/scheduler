...
### Новое поведение ошибок
- При ошибке шага с текстом `AUTH_REQUIRED:*` выполнение немедленно прерывается исключением.
- После выполнения шагов рассчитывается агрегированный статус запуска через `_calculate_run_status(all_step_results)`.
- В итог добавляется `__RUN_STATUS__`.

### Значение `__RUN_STATUS__`
- `success`: все шаги успешно отработали.
- `partial`: есть смешанный результат (часть шагов успешна, часть нет).
- `failed`: критический провал (включая сценарии потери авторизации).
