"""
Скрипт для прямого запуска GoogleSheetsUploadScript

Этот скрипт запускает основной скрипт загрузки в Google-таблицу напрямую, чтобы проверить его работу.
"""
import sys
import os
...
def direct_run():
    """Прямой запуск основного скрипта загрузки в Google-таблицу"""
    print("=== Прямой запуск GoogleSheetsUploadScript ===")
    
    try:
        print("Импорт основного модуля...")
        from scheduler_runner.tasks.reports.GoogleSheetsUploadScript import main
