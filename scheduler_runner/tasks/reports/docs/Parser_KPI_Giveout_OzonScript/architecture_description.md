...
- `BaseOzonParser` - базовый класс для парсинга ОЗОН
- `SCRIPT_CONFIG` из конфигурационного файла
- `REPORT_DATA_SCHEMA` из конфигурации для структуры данных
- Утилиты `configure_logger` для логирования
- Утилиты `load_reports_data` для сохранения данных
- Утилиты `data_transformers` для преобразования данных
...
```
https://turbo-pvz.ozon.ru/outbound/carriages-archive?filter={%22startSentMoment%22:%222026-01-06T00:00%2B03:00%22,%22endSentMoment%22:%222026-01-06T23:59%2B03:00%22,%22flowType%22:%22Direct%22}
```
...
Скрипт использует следующие селекторы из конфигурации:
- `PVZ_INPUT` - для установки пункта выдачи
- `PVZ_INPUT_READONLY` - для извлечения информации о ПВЗ
- `PVZ_INPUT_CLASS_READONLY` - для извлечения информации о ПВЗ
- `TOTAL_GIVEOUT` - для извлечения общего количества выдач
- `GIVEOUT_COUNT` - для извлечения количества выданных посылок ("Всего: N")