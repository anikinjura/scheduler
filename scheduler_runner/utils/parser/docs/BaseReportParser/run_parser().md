...
### Новое поведение
- После мультишаговой обработки метод ожидает служебный флаг `__RUN_STATUS__` в `data`.
- Если `__RUN_STATUS__ == "failed"`, метод выбрасывает исключение и завершает запуск как неуспешный.

### Практический эффект
- Парсер больше не считается успешным при критических сбоях авторизации, даже если часть шагов вернула данные.
