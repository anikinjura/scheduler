...
try:
    # Импортируем конфигурацию путей из base_config
    from config.base_config import PATH_CONFIG, BASE_DIR
except ImportError as e:
    print(f"Ошибка импорта base_config: {e}")
    # Используем значения по умолчанию
    BASE_DIR = PROJECT_ROOT
    PATH_CONFIG = {
        'SCRIPTS_DIR': PROJECT_ROOT / 'scheduler_runner' / 'tasks',
...
def _setup_logger():
    """Настройка логгера для schedule_config."""
    logger = logging.getLogger('schedule_config')
...
DEFAULT_TASK_ENV: Dict[str, Optional[str]] = {
    'PYTHON_PATH': str(BASE_DIR),
    'LOG_LEVEL': 'INFO',
    'TASK_RUNNER': 'scheduler_runner',
    'SCRIPTS_DIR': str(PATH_CONFIG.get('SCRIPTS_DIR', '')),
    'LOGS_ROOT': str(PATH_CONFIG.get('LOGS_ROOT', '')),
...
def _discover_task_configs() -> List[Dict[str, Any]]:
    ...
    # Проверяем существование директории задач
    ...
    for task_dir in tasks_dir.iterdir():
        ...
        if not task_dir.is_dir() or task_dir.name.startswith('_'):
            logger.debug(f"Пропускаем: {task_dir.name}")
            continue
...
def _validate_schedule_config(schedule: List[Dict[str, Any]]) -> None:
    """
...
def get_task_env(task: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Объединяем переменные, преобразуя все значения в строки
...
try:
    # Обнаруживаем и загружаем конфигурации задач
