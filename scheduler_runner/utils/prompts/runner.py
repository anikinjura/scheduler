...
def filter_tasks(
    ...
    Example:
        ...
        1
    """
    # Первый этап: фильтрация по пользователю
    user_tasks = [task for task in all_tasks if task.get('user') == user]
    
    # Второй этап: дополнительная фильтрация по имени задачи (если указано)
    if task_name:
        user_tasks = [task for task in user_tasks if task.get('name') == task_name]
...
def execute_task(
    ...
    # Если не время для выполнения, это не ошибка
    if not should_run:
        return True
